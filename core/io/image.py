...
from core.pad.numpy import pad_image_bottom_right
...
def load_image(image_path,
               ...
               df=1,
               padding=False,
               interp=cv2.INTER_AREA):  # read image
    ...
    if resize_fn != "None" and resize is not None:
        # max
        if resize_fn == "max":
            w_new, h_new = get_target_wh(original_size[0], original_size[1],
                                         resize, resize_fn=max)
        elif resize_fn == "min":
            w_new, h_new = get_target_wh(original_size[0], original_size[1],
                                         resize, resize_fn=min)
        else:
            w_new, h_new = resize
    else:
...
def load_image_tensor(image_path,
    ...
    # load image
    cv_img, mask, scale, original_size = load_image(image_path,
                                                    resize=resize,
                                                    resize_fn=resize_fn,
                                                    gray=gray,
                                                    df=df,
